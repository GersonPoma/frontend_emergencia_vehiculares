...
import { Usuario } from 'src/app/models/cuentas/Usuario.model';
import { Rol } from 'src/app/models/cuentas/rol.model';
...
export class UsuarioComponent implements OnInit, OnDestroy {
  displayedColumns: string[] = ['id', 'username', 'rol_id', 'acciones'];
  dataSource: Usuario[] = [];
    ...
    this.breakpointObserver.observe([Breakpoints.Handset])
      ...
      .subscribe(result => {
        ...
        this.displayedColumns = result.matches
          ? ['username', 'rol_id', 'acciones']
          : ['id', 'username', 'rol_id', 'acciones'];
      });
    this.loadRoles();
    this.loadUsuarios();
  }  
  ...
  loadRoles(): void {
    const rolesUrl = this.configService.getApiUrl('roles');
    this.apiService.getWithPagination<Rol>(rolesUrl, 1, 100)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data: Pagination<Rol>) => {
          data.datos.forEach(r => this.rolesMap.set(r.id, r.nombre));
        }
      });
  }

  getRolNombre(rol_id: number): string {
    return this.rolesMap.get(rol_id) || `Rol #${rol_id}`;
  }
